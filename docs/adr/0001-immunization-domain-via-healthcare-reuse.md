...
4. **Fase 4** (~7 dias): RNDS Settings + cliente FHIR R4 + auto-envio + retry scheduler + **`resolve_cns(cpf)`** via `GET /patient` + envio do `Immunization` (CPF como identifier).
   ...
   - **4c — CONCLUÍDA (núcleo)**: `rnds_immunization.build_immunization_bundle` monta o Bundle FHIR `document` (Composition + Immunization) do perfil RIA — vaccineCode (BRImunobiologico via `Medication.codigo_rnds`), patient (CNS→CPF fallback), occurrence, lote, fabricante, dose, performer (CNS do profissional), location (CNES), extensão de estratégia de vacinação. `enviar_imunizacao` faz `POST /Bundle` e grava `rnds_status/rnds_id/rnds_payload` na Drug Prescription. Hook `Patient Encounter.on_update` enfileira (assíncrono, queue long) o envio das vacinas — **automático** (registro compulsório, sem aprovação manual diferente do WhatsApp). Retry `hourly` para `rnds_status=Erro`. Custom field `Healthcare Practitioner.cns`. **Pendente de validação real**: ValueSets/códigos auxiliares (estratégia, grupo de atendimento, via, CBO, tipo de documento) usam defaults — confirmar contra o RNDS de homologação com os valores oficiais do SIPNI.
